...
def fread(frame: int):
    ...
    hdcolor = []
    mf = math.floor
    YUV = np.dot(im, YUVmatrix) #convert to YUV and 2x2 squares
    for p in range(0, height * 40):
        TL=YUV[(p*2)//160*2][(p*2)%160][0] #top left pixel of square
        TR=YUV[(p*2)//160*2][(p*2+1)%160][0] #top right pixel of square
        BL=YUV[(p*2)//160*2+1][(p*2)%160][0] #bottom left pixel of square
        BR=YUV[(p*2)//160*2+1][(p*2+1)%160][0] #bottom right pixel of square
        bl = (TL+TR+BL+BR)/4  #average value of square
        i = YUV[(p*2)//160*2][(p*2)%160][1]
        q = YUV[(p*2)//160*2][(p*2)%160][2]
        if bl == 0:
            lr = 0
            tb = 0
            ch = 0
            ...
            lr = (TL-TR+BL-BR)/bl #left/rigt difference
            tb = (TL+TR-BL-BR)/bl #top/bottom difference
            ch = (TL-TR-BL+BR)/bl #checkerboarded difference
    ...
    text = ""
    for n in range(0, height * 160): #convert everything to text
    ...
    obj = f'</object></c><c type="58"><object id="{frame*3+16317}" n="frame_{frame}a" v="{text[0:8000]}"><pos x="{xpos}" y="{ypos}"/></object></c><c type="58"><object id="frame*3+16318" n="frame_{frame}b" v="{text[8000:16000]}"><pos x="{xpos}" y="{ypos}"/></object></c><c type="58"><object id="frame*3+16319" n="frame_{frame}c" v="{text[16000:24000]}"><pos x="{xpos}" y="{ypos}"/>'
